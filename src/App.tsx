...
import { StrategyMode } from './modes/StrategyMode';
import { SimulationMode } from './modes/SimulationMode';

type AppMode = 'strategy' | 'simulation';
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-tactical-bg-primary">
      {/* Mode Selector */}
      <div className="bg-tactical-bg-secondary border-b-2 border-tactical-olive-700">
        <div className="max-w-[1800px] mx-auto px-6 py-4">
          <div className="flex items-center gap-4">
            ...
            <button
              ...
              className={`
                px-6 py-3 rounded-lg font-bold text-sm transition-all flex items-center gap-3
                ${mode === 'strategy'
                  ? 'bg-tactical-olive-700 text-white border-2 border-tactical-olive-500'
                  : 'bg-black/30 text-gray-500 border-2 border-transparent hover:border-gray-700'
                }
              `}
            >
              <Brain className="w-5 h-5" />
              <div className="text-left">
                <div className="font-bold">STRATEGY MODE</div>
                <div className="text-[10px] opacity-70">Single Decision Analysis</div>
              </div>
            </button>
            ...
            <button
              ...
              className={`
                px-6 py-3 rounded-lg font-bold text-sm transition-all flex items-center gap-3
                ${mode === 'simulation'
                  ? 'bg-tactical-blue/80 text-white border-2 border-tactical-blue'
                  : 'bg-black/30 text-gray-500 border-2 border-transparent hover:border-gray-700'
                }
              `}
            >
              ...
              <div className="text-left">
                ...
                <div className="text-[10px] opacity-70">Full Game Playthrough</div>
              </div>
            </button>
            ...
            <div className="flex-1 text-right">
              <p className="text-xs text-gray-500">
                {mode === 'strategy' 
                  ? '🎯 Analyze battlefield situation and get strategic recommendation'
                  : '🎮 Play full tactical game with turn-based combat'
                }
              </p>
            </div>
          </div>
      ...
      {mode === 'strategy' ? <StrategyMode /> : <SimulationMode />}
    </div>
  );
}
