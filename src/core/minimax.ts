...
/**
 * 🔥 NEW: Terrain-aware scoring
 */
function getTerrainAdjustedScore(state: BattleState): number {
  let score = heuristic(state);

  const [aiX, aiY] = state.aiUnit.position;
  const [enemyX, enemyY] = state.enemyUnit.position;

  const aiTerrain = state.terrain[aiX][aiY].type.toUpperCase();
  const enemyTerrain = state.terrain[enemyX][enemyY].type.toUpperCase();
  ...
  // AI gets bonus
  score += terrainBonus[aiTerrain] || 0;

  // Enemy advantage reduces AI score
  score -= terrainBonus[enemyTerrain] || 0;
...
export function minimax(
  ...
  if (isTerminal) {
    return {
      score: getTerrainAdjustedScore(state), // 🔥 FIXED HERE
      action: null,
      nodesEvaluated: nodesCount.value,
    };
  }
